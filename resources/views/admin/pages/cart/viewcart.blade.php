    ...
    <div class="col-lg-8">
      ...
      @guest
        <div class="alert alert-info mt-3">
          Please <a href="{{ route('login', ['intended' => route('cart.checkout')]) }}" class="alert-link">log in</a> to proceed to checkout.
        </div>
      @endguest
      @if ($errors->any())
      ...
      @php
        ...
        $resolveThumb = function ($path) use ($placeholder) {
            ...
            // Serve directly if already web-visible
            if (
            ...
            // Default: assume file is in storage/app/public
                  ...
                  <tr>
                    ...
                    <td class="text-center">
                      <div class="d-inline-flex align-items-center gap-2">
                        {{-- minus: decrement by 1 (or remove if hits 0) --}}
                        ...
                        {{-- plus: add 1 using existing add route --}}
                      ...
                      @if(!empty($row['available']))
                        <div class="text-muted small mt-1">Available: {{ $row['available'] }}</div>
                      @endif
                    </td>
      ...
      <div class="card shadow-sm">
        <div class="card-body">
          ...
          <div class="d-grid mt-3">
            @if(count($cart))
              @auth
                <a href="{{ route('cart.checkout') }}" class="btn btn-primary btn-lg rounded-pill">
                  Check Out
                </a>
              @else
                <a href="{{ route('login', ['intended' => route('cart.checkout')]) }}" class="btn btn-primary btn-lg rounded-pill">
                  Login to Checkout
                </a>
              @endauth
            @else
